...
- **Automação no cluster:** CronJobs e consumer Slack em [k8s/orchestrator/](../k8s/orchestrator/) — digest diário (18:00 UTC), check-timers cosmético (a cada 10 min), consensus-loop-runner (a cada 2 min). Aplicar: `make orchestrator-configmap`, criar Secret `orchestrator-slack` (app/canal próprios do orquestrador), depois `make orchestrator-apply` ou `make up`. Ver [k8s/orchestrator/README.md](../k8s/orchestrator/README.md).
