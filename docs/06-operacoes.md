...
- **Stream `orchestrator:events`:** Eventos como `trigger_architect_fallback`, `issue_back_to_po`, `consensus_loop_requested` são publicados neste stream. Um **consumidor** ([scripts/consumer_orchestrator_events_slack.py](../scripts/consumer_orchestrator_events_slack.py)) lê o stream e envia cada evento ao Slack (canal de alertas).
- **Envio ao Slack:** [scripts/slack_notify.py](../scripts/slack_notify.py) — usa `SLACK_WEBHOOK_URL` (Incoming Webhook) ou `SLACK_BOT_TOKEN` + `SLACK_ALERTS_CHANNEL_ID`. Variáveis em [.env.example](../.env.example).
- **Alerta imediato (freio de mão):** Quando o orquestrador aciona o freio de mão por orçamento de degradação, além de escrever o relatório ele **envia uma mensagem imediata** ao Slack (chamada direta a `slack_notify.py`).
...
- **Automação no cluster:** CronJobs e consumer Slack em [k8s/orchestrator/](../k8s/orchestrator/) — digest diário (18:00 UTC), check-timers cosmético (a cada 10 min), consensus-loop-runner (a cada 2 min). Aplicar: `make orchestrator-configmap`, criar Secret `orchestrator-slack` com variáveis Slack, depois `make orchestrator-apply` ou `make up`. Ver [k8s/orchestrator/README.md](../k8s/orchestrator/README.md).
