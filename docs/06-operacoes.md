...
2. **Limpar o lock do Redis** (caso o script de GPU Lock tenha ficado travado):
   ```bash
   kubectl exec -it redis-master-0 -- redis-cli DEL gpu_active_lock
   ```
   (Ajustar o nome do pod do Redis conforme o deployment, ex.: `redis-master-0` ou o que constar em `kubectl get pods`.)
...
- **Hard timeout no Kubernetes (lock de GPU):** Primeira linha de defesa contra lock órfão (processo morre sem liberar). Configurar os pods que usam GPU com tempo máximo de execução (ex.: 120 s); ao exceder, o K8s mata o pod e a tarefa volta à fila. Recuperação manual (limpar `gpu_active_lock` no Redis) é **último recurso** quando o timeout automático não for suficiente. Ver [04-infraestrutura.md](04-infraestrutura.md) e [scripts/gpu_lock.md](scripts/gpu_lock.md).
