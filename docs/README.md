...
| [37-deploy-fase0-telegram-ceo-ollama.md](37-deploy-fase0-telegram-ceo-ollama.md) | **Deploy Fase 0** — CEO via Telegram + Ollama no Kubernetes; passos de apply, secret a partir do [.env.example](../.env.example), workspace por agente, pairing. |
| [42-slack-tokens-setup.md](42-slack-tokens-setup.md) | **Slack — tokens e um app por agente** — Onde achar tokens (OpenClaw Socket Mode); variáveis por agente em [.env.example](../.env.example); validação: agentes no Slack (Marina/PO, Ricardo/CEO, Lucas/Developer, Rafael/QA, etc.) com SOUL correto. |
