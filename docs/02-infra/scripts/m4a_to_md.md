...
Converte arquivo de áudio M4A em texto (transcrição) e salva em arquivo Markdown. **Offline e sem custo de API** para transcrição, usando **faster-whisper** (recomendado) ou **openai-whisper**. Idioma forçado em português do Brasil (PT-BR).

## Requisitos

- Python 3
- `ffmpeg` instalado no sistema
- Um dos backends: `pip install faster-whisper` ou `pip install openai-whisper`

## Uso na linha de comando

```bash
# Transcrever com modelo padrão (base)
python m4a_to_md.py arquivo.m4a

# Especificar arquivo de saída
python m4a_to_md.py arquivo.m4a -o saida.md

# Modelo maior (mais preciso, mais lento)
python m4a_to_md.py arquivo.m4a -m small

# Forçar CPU ou GPU
python m4a_to_md.py arquivo.m4a -d cuda
python m4a_to_md.py arquivo.m4a -d cpu
```
...
|------------|------------------------------|--------|
| `arquivo`  | Caminho do arquivo M4A       | —      |
| `-o`, `--output` | Arquivo .md de saída   | mesmo nome do áudio |
| `-m`, `--model`   | Modelo Whisper: tiny, base, small, medium, large | base |
| `-d`, `--device`  | auto, cuda, cpu              | auto   |

## Uso no OpenClaw

O setup configura o OpenClaw para usar este script como comando de voz-para-texto no canal Telegram, por exemplo:
...
O áudio recebido é salvo temporariamente; o OpenClaw invoca o script e envia o texto transcrito ao Agente CEO.
