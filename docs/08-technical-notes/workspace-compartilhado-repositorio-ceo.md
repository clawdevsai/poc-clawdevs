# Investigação: repositório “baixado pelo CEO” não aparece em ~/clawdevs-shared/workspace

## Contexto

O CEO (via Slack) informou ter baixado o repositório e analisado a documentação. No host, `~/clawdevs-shared/workspace` estava vazio (`tree` → 0 directories, 0 files). Investigação para entender onde o clone iria parar e por que não aparece onde o Diretor espera.
...
3. **Clone sem caminho:** `git clone https://github.com/clawdevsai/clawdevs` criaria **`/workspace/ceo/clawdevs`** (ou nome do repo), não em `/workspace/workspace`.
4. **No host:** `/workspace` no pod = `~/clawdevs-shared` no host. Logo:
   ...
   - A pasta **`~/clawdevs-shared/workspace`** no host corresponde a **`/workspace/workspace`** no pod; essa pasta **não é criada** pelo init e só existe se algum agente criar e clonar aí.
...
| Causa | Explicação |
|-------|------------|
| **Clone no workspace do agente** | CEO clonou em `/workspace/ceo` → aparece em `~/clawdevs-shared/ceo/`, não em `~/clawdevs-shared/workspace/`. |
...
Para que repositórios clonados pelo CEO apareçam em **`~/clawdevs-shared/workspace`**, o CEO deve clonar **explicitamente** em `/workspace/workspace`:

- Exemplo: `git clone https://github.com/clawdevsai/clawdevs /workspace/workspace/clawdevs`
...
- `k8s/management-team/openclaw/README.md`: estrutura do host (`workspace/` = repositórios clonados).
