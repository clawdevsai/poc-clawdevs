...
- **Falta sobretudo:** (1) **validação em ambiente real** (124, 019, phase2, orquestrador no cluster); (2) **pre-flight e validação reversa** efetivamente chamados pelo pipeline que gera resumo (adapter já faz pre-flight em /publish-to-cloud); (3) **Fases 5–7** (Memória Elite no cluster, ferramentas) e **validação/priorização** do PO para o que já está no repo. **Fases 8–11** têm documentação e entregáveis implementados no repositório: skills/ambiente (090–093, CronJob auto-update 092, FreeRide/fluxo PO), integrações (Secret Maton 100, dados/Exa/headless 101–103), frontend (110–111 em SOUL), War Room (mock + doc 120), Chaos (manual de falhas 121), balanceamento GPU/CPU (doc 122).
...
| **Fases 6–11** | Habilidades transversais, ferramentas (browser, summarize, gh, OpenCode Controller), skills (Fase 8: docs + CronJob 092 + SOUL), API Gateway Maton (Secret + 09-setup), Exa/headless/dados (docs), frontend (SOUL 23/32), War Room (mock + doc), Chaos (manual de falhas), balanceamento (doc). Conforme backlog; priorização do PO para ativar/validar em ambiente. |
