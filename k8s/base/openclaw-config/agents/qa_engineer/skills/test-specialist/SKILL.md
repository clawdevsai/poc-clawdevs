...
This skill is restricted to project work inside `/data/openclaw/projects` only.

- You may read agent context files in `/data/openclaw` when required by startup/session rules.
- Only write, execute, and modify project files within `/data/openclaw/projects`.
- Refuse operations that target paths outside `/data/openclaw/projects`.
- If the requested project is outside this directory, ask the user to move or point the project to an allowed path first.
- Do not run commands that can affect global/system paths when they are not required for work inside the allowed directory.
