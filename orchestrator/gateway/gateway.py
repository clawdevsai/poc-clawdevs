...
class ContextTruncator:
    ...
    def truncate(self, payload: str, keep_header_chars: int = 500) -> tuple[str, bool]:
        ...
        tail = payload[-(max_chars - keep_header_chars):]
...
class DegradationBudget:
    ...
    def _trigger_consensus_loop(self, ratio: float):
        ...
        r = self.r
        r.xadd("degradation:consensus_trigger", {
            "ratio": str(ratio),
    ...
    def trigger_emergency_brake(self):
        ...
        logger.critical("FREIO DE MÃO ativado. Esteira pausada. Aguardando comando explícito de desbloqueio.")
...
class HeadblessClusterWatchdog:
    ...
    def _trigger_headless_failsafe(self):
        ...
        self.r.xadd("devops:emergency", {
            "action": "headless_failsafe",
            "branch": branch_name,
            "timestamp": str(int(time.time())),
            "reason": "ceo_timeout",
        })
        logger.critical("Contingência cluster acéfalo: branch efêmera '%s' solicitada.", branch_name)
    ...
    def _trigger_auto_resume(self):
        ...
        logger.info("Retomada automática acionada (3 ciclos estáveis). Nenhuma intervenção humana necessária.")
...
class Gateway:
    ...
    def process_strategy_event(self, event: dict) -> Optional[dict]:
        ...
        if vfm_data:
            ...
            if not vfm.approved:
                logger.info("VFM Score negativo (%.2f < %.2f): evento %s descartado antes do Gateway.",
                            vfm.net_score, vfm.threshold, event_id)
                return None

        # 2. Token Bucket
        if not self.token_bucket.allow():
            self.r.xadd("ceo:rejected", {
                "event_id": event_id,
