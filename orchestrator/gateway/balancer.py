...
class DynamicBalancer:
    ...
    def decide_placement(self, agent_role: str, task_priority: int = 1) -> str:
        ...
        if state.daily_budget_ratio >= 1.0:
            logger.warning(
                "BUDGET EXCEDIDO (%d%%). Forçando local unicamente.",
                state.daily_budget_ratio * 100,
            )
        ...
        if state.efficiency_ratio < self.threshold_efficiency:
            if agent_role in ["CEO", "PO"]:
                logger.info(
                    "BAIXA EFICIÊNCIA (%.1f%%). Movendo CEO/PO para CPU.",
                    state.efficiency_ratio * 100,
                )
                return "cpu"
    ...
    def get_model_for_tier(self, tier: str, agent_role: str) -> str:
        ...
        if tier == "cloud":
            if agent_role == "CEO":
                return "openrouter/google/gemini-2.0-pro-02-05:free"
            return "openrouter/google/gemini-2.0-flash-001:free"

        if tier == "gpu":
            if agent_role in ["Developer", "DBA"]:
