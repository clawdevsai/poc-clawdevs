...
# Uso: ./scripts/ops/slack-openclaw-check.sh
set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
SECRET_SCRIPT="$SCRIPT_DIR/../openclaw/k8s-openclaw-secret-from-env.sh"
cd "$REPO_ROOT"
...
if [[ "${HAS_APP:-0}" -lt 10 || "${HAS_BOT:-0}" -lt 10 ]]; then
  ...
  echo "   O pod NÃO lê o .env do seu PC; defina OPENCLAW_SLACK_APP_TOKEN e OPENCLAW_SLACK_BOT_TOKEN no .env e rode: $SECRET_SCRIPT"
