...
# Não imprime nem loga o token. Uso: ./scripts/test_github_access.sh [host|cluster|all]
# Ref: .env (GH_TOKEN, GITHUB_TOKEN), k8s/development-team/secret-github.example.yaml, docs/20-ferramenta-github-gh.md
set -e
...
test_host() {
  ...
  if [ -f "$REPO_ROOT/.env" ]; then
    # Carrega apenas as linhas de token (evita side effects de outros vars)
...
test_cluster() {
  ...
  # Pod que monta o secret (openclaw é o gateway usado por todos)
  for deploy in openclaw; do
    ...
    # Teste de API a partir do container (curl é comum em imagens node)
