...
# e ser identificável como criada pelo agente (autor = conta do token).
# Pode rodar no host (.env) ou no cluster (pod openclaw = agente). Por padrão deixa a issue aberta.
# Uso:
#   ./scripts/test_github_create_issue.sh              # host, issue aberta
#   RUN_IN_CLUSTER=1 ./scripts/test_github_create_issue.sh   # agente (pod) cria a issue
#   CLOSE_ISSUE=1 ./scripts/test_github_create_issue.sh      # fecha a issue depois
# Ref: .env (GITHUB_TOKEN/GH_TOKEN), docs/20-ferramenta-github-gh.md
set -e
...
# Título e corpo que identificam o agente como criador
...
if [ "$CLOSE_ISSUE" = "1" ]; then
  ...
  echo "  Issue deixada aberta. Para fechar depois: CLOSE_ISSUE=1 ./scripts/test_github_create_issue.sh (ou feche manualmente no GitHub)."
fi
