...
# Tudo roda no K8s; não usa mais config/openclaw/workspace-ceo no host.
# Uso:
#   make reset-memory
#   RESET_REDIS=0 ./scripts/reset_agent_memory.sh   # só restart openclaw
#   RESET_REPO=0 ./scripts/reset_agent_memory.sh    # só Redis
#   REDIS_HOST=127.0.0.1 REDIS_PORT=6379 ./scripts/reset_agent_memory.sh  # Redis via port-forward
set -e
...
# Lua (uma linha): SCAN + UNLINK para project:v1:* (não bloqueia como KEYS)
